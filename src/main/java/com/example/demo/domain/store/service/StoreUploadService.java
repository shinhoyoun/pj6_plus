...
public class StoreUploadService {
    ...
    @Async
    public void uploadCsvFile(File file) throws Exception {
    ...
    public void saveAllStore(int start, int end) {
        ...
        try {
            ...
            for (JsonNode itemNode : rowNode) {
                try {
                    String companyName = itemNode.path("COMPANY").asText(null);

                    if (storeRepository.existsByCompanyName(companyName)) {
                        continue;
                    }
                    ...
                    log.error("Error processing OpenAPI item: {}", e.getMessage());
                }
            }
